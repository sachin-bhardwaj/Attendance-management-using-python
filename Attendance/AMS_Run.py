...
def manually_fill():
    ...
    def fill_attendance():
        ...
        global subb
        subb=SUB_ENTRY.get()
        db = str(subb + "_" + Date + "_Time_" + Hour + "_" + Minute + "_" + Second)
        ...
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='',port=3306, database='manually_fill_attendance')
        ...
        if subb=='':
            ...
            def enter_data_DB():
                ...
                if ENROLLMENT=='':
                    ...
                    Insert_data = "INSERT INTO " + db + " (ID,ENROLLMENT,NAME,DATE,TIME) VALUES (0, %s, %s, %s,%s)"
            ...
            def create_csv():
                ...
                csv_name='G:/Attendance/Attendance/Manually'+db+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"G:\Attendance\Attendance"')
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                ##Create table for Attendance
                date_for_DB = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m_%d')
                db = str( Subject + "_" + date_for_DB + "_Time_" + Hour + "_" + Minute + "_" + Second)
                import pymysql.connections

                ###Connect to the database
                try:
                    global cursor
                    connection = pymysql.connect(host='localhost', user='root', port=3306,password='', database='manually_fill_attendance')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                sql = "CREATE TABLE " + db + """
